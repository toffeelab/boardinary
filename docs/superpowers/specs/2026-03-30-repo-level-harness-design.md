...
| -------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `jq` 미설치          | hook non-zero exit → 차단             | `common.sh`의 `HOOK_FAIL_OPEN` 분기: PreToolUse=차단(exit 2)+설치안내, PostToolUse=경고+통과(exit 0)                                             |
| permission denied    | 실행 불가 → 차단                      | `init-harness.sh`에서 `chmod +x` 자동 적용. README에 수동 방법 안내                                                                              |
| 무한루프/타임아웃    | Claude Code hook 타임아웃 (기본 10초) | 복잡한 로직 지양. 단순 패턴 매칭만 수행                                                                                                          |
| stdin JSON 파싱 실패 | 변수 비어있음                         | `common.sh`의 `HOOK_FAIL_OPEN` 분기: PreToolUse=**차단**(exit 2), PostToolUse=경고+**통과**(exit 0). 보안 hook은 파싱 불가 시 차단이 안전한 방향 |
